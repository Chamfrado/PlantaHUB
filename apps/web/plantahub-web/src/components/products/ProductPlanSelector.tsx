        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_380px]">
          <div className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="rounded-2xl border border-orange-100 bg-white px-4 py-2 text-sm font-semibold text-primary-600">
                {selectedCodes.length} selecionado(s)
              </div>
            </div>

            {loading ? (
              <div className="mt-6 flex items-center gap-3 rounded-2xl border border-neutral-200 bg-brand-light p-4 text-sm text-brand-muted">
          ...
          <aside className="h-fit lg:sticky lg:top-24">
            <div className="rounded-3xl border border-neutral-200 bg-brand-light p-6 shadow-sm">
              <h3 className="text-xl font-extrabold text-brand-black">Resumo da compra</h3>
                  ...
                  selectedItems.map(item => (
                    ...
                    >
                      <div>
                        ...
                        <div className="mt-1 text-xs text-brand-muted">{item.code}</div>
                      </div>
              ...
              <div className="mt-5 grid gap-3">
                ...
                <button
                  ...
                  onClick={onAddToCart}
                  disabled={!canSubmit}
                  className="inline-flex items-center justify-center gap-2 rounded-2xl border border-neutral-300 bg-white px-5 py-3 font-semibold text-brand-black transition hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {submitting === 'cart' ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
